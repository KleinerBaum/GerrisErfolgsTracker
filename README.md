...
- Hinweisboxen sind im Sidebar-Panel **Sicherheit & Daten / Safety & data** per Toggle **Hinweise anzeigen / Show safety notes** standardmäßig ausgeblendet, lassen sich bei Bedarf aber einblenden; der Speicherhinweis oben im Panel wird separat über **Speicherhinweis anzeigen / Show storage notice** gesteuert (Standard: aus) / The **Sicherheit & Daten / Safety & data** sidebar panel now hides safety and storage notices by default behind the **Hinweise anzeigen / Show safety notes** toggle, while the storage notice above the title remains separately controlled via **Speicherhinweis anzeigen / Show storage notice** (default: off).
