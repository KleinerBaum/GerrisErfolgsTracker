...
Die Sidebar konzentriert sich auf die Navigation; Schalter, Build-Infos sowie Sicherheits- und Gamification-Optionen liegen nun in einem kompakten Header-Dropdown rechts neben „Journal“ / The sidebar now focuses on navigation, while toggles, build info, and safety/gamification options live in a compact header dropdown placed to the right of “Journal”.
  ...
  - Dashboard-Kopf ist kompakter gestaltet: reduzierte Abstände, breitere Quick-Action-Spalten und ikonische Labels (📝/🎯/📓) halten Titel und Schnellaktionen in einer Zeile; auf schmalen Viewports brechen die Spalten sauber um / The dashboard header is now more compact with tighter spacing, wider quick-action columns, and icon-first labels (📝/🎯/📓) to keep the title and quick actions on one line while gracefully wrapping on narrow viewports.
...
- **Build-Info im Header-Dropdown:** Die App zeigt den aktuellen Commit (Kurz-Hash + Datum) im Einstellungen-Popover rechts
  neben „Journal“ an, damit Deployments sofort mit dem erwarteten `main`-Stand abgeglichen werden können / The app surfaces the
  current commit (short hash + commit date) inside the settings popover next to “Journal” so deployments can be compared against
...
- Die Seitenleiste bündelt die Navigation zwischen **Dashboard**, **Ziele / Goals**, **Aufgaben / Tasks** und **Tagebuch / Journal**, während Gamification-Variante, AI-Schalter, Sicherheitsoptionen und Build-Info in das Header-Dropdown rechts neben **Journal** gewandert sind. Die App startet mit geöffneter Sidebar und lädt standardmäßig das **Dashboard** / The sidebar hosts navigation across **Dashboard**, **Goals**, **Tasks**, and **Journal**, while the gamification mode, AI toggle, safety controls, and build info live in the header dropdown next to **Journal**. The app still opens with the sidebar expanded and defaults to the **Dashboard**.
...
- Auf der Seite **Dashboard** startet jetzt ein fünffach gespaltener Tachometer-Überblick zu Stellensuche, Administratives, Familie & Freunde, Drogen sowie Tagesstruktur; rechts daneben steuerst du per Checkbox, ob das KPI-Dashboard bzw. die Kategoriendiagramme eingeblendet werden.
- Im Dashboard-Block **Ziele im Überblick** steuerst du im Expander **Einstellungen**, welche Kategorien als Tachometer erscheinen und passt die Kennzahlen an (Anzahl angezeigter KPIs, Auswahl/Setup einzelner Kennzahlen, Farbe oder Darstellungsart). Jeder Tacho ist anklickbar und öffnet darunter eine Detailansicht mit den zugeordneten Aufgaben. / On the dashboard, the **Ziele im Überblick / Goals overview** block contains a **Einstellungen / Settings** expander where you pick which categories should be shown as gauges and adjust the metrics (number of KPIs shown, metric selection/configuration, color or visualization style). Each gauge is clickable and opens a detailed task view below.
...
- Drop-down für Gamification-Modus (Punkte, Abzeichen oder ein neuer Avatar-Modus mit motivierenden Sprüchen) über das Dashboard bzw. den Header; Fortschritt, Level und Motivation erscheinen im Gamification-Panel der Sidebar.
