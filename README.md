...
- Oben im Bereich **Ziele** stehen die Aktions-Buttons **Ziel erstellen / Create goal**, **Gelöst / Completed**, **Aufgabe erstellen / Create task** und **Tagebucheintrag erstellen / Create journal entry** nebeneinander in derselben Primärfarbe für ein konsistentes Erscheinungsbild.
...
- Im Block **Ziele im Überblick** steuerst du im Expander **Einstellungen**, welche Aufgaben in den KPIs und Kategorien berücksichtigt werden (z. B. nur Fokus-Tasks) und passt die Tachometer an (Anzahl angezeigter KPIs, Auswahl/Setup einzelner Kennzahlen, Farbe oder Darstellungsart) / In the **Ziele im Überblick / Goals overview** block, the **Einstellungen / Settings** expander lets you choose which tasks count toward KPIs and categories (e.g., focus tasks only) and adjust the gauges (number of KPIs shown, metric selection/configuration, color or visualization style).
- Der Bereich **Aktivität abhaken / Log activity** auf der Seite **Ziele** bietet jetzt drei Aktionen nebeneinander: **Gelöst / Completed** blendet erst nach Klick das Auswahlfeld für offene Aufgaben ein, **Aufgabe erstellen / Create task** öffnet das gewohnte ToDo-Popup, und **Tagebucheintrag erstellen / Create journal entry** startet ein kompaktes Journal-Popup; KPI-Karten, Tachometer und Gamification-Popups werden nach Abschlüssen direkt aktualisiert / The **Log activity** section on the **Goals** page now shows three side-by-side actions: **Gelöst / Completed** reveals the open-task dropdown only after clicking, **Aufgabe erstellen / Create task** opens the familiar ToDo popup, and **Tagebucheintrag erstellen / Create journal entry** launches a compact journal popup; KPI cards, gauges, and gamification popups still refresh immediately after completions.
- Unter den Kennzahlen und Kategorien bindet die Seite **Ziele** den Google-Kalender **2025 von Carla, Miri & Gerrit** ein, damit Termine sichtbar bleiben / Below the KPI and category sections, the **Goals** page embeds the Google Calendar **2025 by Carla, Miri & Gerrit** so dates stay visible.
