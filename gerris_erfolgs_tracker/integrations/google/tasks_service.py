...
REQUIRED_SCOPES: tuple[str, ...] = (*BASE_SCOPES, GOOGLE_SCOPE_TASKS_READONLY)
...
def _to_task_item(item: dict[str, Any]) -> TaskItem:
    return TaskItem(
        ...
        updated=parse_google_datetime(
            item.get("updated") if isinstance(item.get("updated"), str) else None
        ),
    )
